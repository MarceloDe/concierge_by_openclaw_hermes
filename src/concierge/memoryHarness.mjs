  ...
  if (latestMessage) {
    ...
    retained.push(
      await upsertMemoryItem(store, {
        ...
        adapter_status: "postgres_source_pointer_ready_for_zep_graphiti",
  ...
  if (managed.state) {
    retained.push(
      await upsertMemoryItem(store, {
        ...
        adapter_status: "langgraph_checkpointer_active",
...
export async function buildContextPacket(store, { user, session = null, channel = "local_web_chat", userInput = "" }) {
  ...
  const packet = {
    ...
    adapterChoice: {
      ...
      longTermProductMemory: "zep_graphiti",
      graphitiRole: "langgraph_recall_before_graph_and_retain_after_graph_when_enabled",
      openClawRole: "read_only_context_projection_and_bounded_worker_runtime_state"
    },
    user: {
      id: user.id,
      name: user.name,
