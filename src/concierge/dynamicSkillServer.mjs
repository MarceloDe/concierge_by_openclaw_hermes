...
export async function resolveDynamicSkillContext(store, state, options = {}) {
  ...
  const context = {
    ...
    workflow: state.workflow ?? state.llm_orchestration_decision?.classification?.workflow ?? null,
    intent: state.llm_orchestration_decision?.classification?.intent ?? state.llm_orchestration_decision?.classification?.taskClass ?? state.intent ?? null,
