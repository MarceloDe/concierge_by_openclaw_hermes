...
export async function resolveDynamicSkillContext(store, state, options = {}) {
  ...
  const context = {
    ...
    workflow: state.workflow ?? state.llm_orchestration_decision?.workflow ?? null,
    intent: state.llm_orchestration_decision?.intent ?? state.llm_orchestration_decision?.classification?.taskClass ?? state.intent ?? null,
