...
test("OpenClaw worker job keeps LangGraph as workflow master while empowering task execution", async () => {
  ...
  assert.equal(job.deterministicControls.workerMayReuseRetainedAuthenticatedSandboxSession, true);
  assert.equal(job.deterministicControls.workerMayContinueAfterUserHiddenLiveView, true);
  ...
  assert.ok(job.allowedWork.allowedActions.includes("reuse_retained_authenticated_aws_sandbox_session"));
  assert.ok(job.allowedWork.allowedActions.includes("continue_hidden_read_only_portal_navigation_after_user_login"));
  ...
  assert.equal(job.insuranceSitePlaybook.retainedSessionPolicy.useRetainedAwsSandboxSessionWhenAvailable, true);
  assert.equal(job.insuranceSitePlaybook.retainedSessionPolicy.hideUserLiveViewAfterSuccessfulLogin, true);
  assert.equal(job.insuranceSitePlaybook.retainedSessionPolicy.reopenLiveViewOnlyWhenAuthExpiresOrChallengeAppears, true);
  assert.equal(job.insuranceSitePlaybook.retainedSessionPolicy.rawPasswordStorageAllowed, false);
