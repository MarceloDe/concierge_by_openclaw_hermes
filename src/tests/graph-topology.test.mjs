...
test("LangGraph topology exposes conditional routing boundaries", () => {
  ...
  assert.ok(topology.conditionalEdges.some((edge) => edge.proves.includes("native_hitl_interrupt")));
  ...
  assert.ok(topology.checkpointer);
...
test("conditional edge guards route policy blocks and normal requests differently", () => {
  ...
  assert.equal(routeAfterWorkflowRouter({ final_response: null }), "plan_journey");
  ...
  assert.equal(
    routeAfterEvidenceObservation({ evidence_observation: { nativeLangGraphInterrupt: true }, approval_resume: { ok: false } }),
    "approval_pause"
  );
});
