...
test("remote browser GUI exposes live view and human takeover controls", () => {
  ...
  assert.match(remoteBrowserJs, /providerMode = "local_cdp"/);
  assert.match(remoteBrowserJs, /providerMode === "facade_remote"/);
  assert.match(remoteBrowserJs, /\/api\/v1\/browser\/sessions/);
  assert.match(remoteBrowserJs, /provider,\s*\n\s*options/);
  assert.match(remoteBrowserJs, /remoteViewportOptions/);
  assert.match(remoteBrowserJs, /deviceScaleFactor/);
  assert.match(remoteBrowserJs, /authorization.*Bearer|authHeaders/);
  assert.match(remoteBrowserJs, /Remote AWS browser sandbox/);
  assert.match(remoteBrowserJs, /payload\.data && payload\.mime/);
  assert.doesNotMatch(remoteBrowserJs, /data-viewer/);
  assert.doesNotMatch(remoteBrowserJs, /<iframe/);
  assert.match(remoteBrowserJs, /stage\.addEventListener\("pointerdown"/);
  assert.match(remoteBrowserJs, /stage\.addEventListener\("wheel"/);
  assert.match(remoteBrowserJs, /Remote AWS browser live/);
  ...
  assert.match(remoteBrowserJs, /mode: "request"/);
  assert.match(remoteBrowserJs, /mode: "grant"/);
  assert.match(remoteBrowserJs, /mode: "end"/);
  assert.match(remoteBrowserJs, /kind: "wheel"/);
  assert.match(remoteBrowserJs, /kind: "navigate"/);
  assert.match(remoteBrowserJs, /data-browser-url/);
  assert.match(remoteBrowserJs, /Expand browser/);
  assert.match(remoteBrowserJs, /tabindex="0"/);
  assert.match(remoteBrowserJs, /focusControlSurface/);
  assert.match(remoteBrowserJs, /scrollIntoView/);
  assert.match(remoteBrowserJs, /approvedBy: "user"/);
  assert.match(remoteBrowserJs, /Fallback relay input only when direct typing does not focus the remote page/);
  assert.match(mvpHtml, /id="workerBrowserPanel"/);
  assert.match(mvpHtml, /id="remoteBrowserMount"/);
  assert.match(mvpHtml, /worker-browser-panel\.is-expanded/);
  assert.match(mvpHtml, /remote-browser__nav/);
  assert.match(mvpHtml, /pointer-events: none/);
  assert.match(mvpHtml, /body\.remote-browser-expanded \.mvp-layout/);
  assert.match(mvpHtml, /overflow-x: auto/);
  assert.match(mvpHtml, /aspect-ratio: auto/);
  assert.doesNotMatch(mvpHtml, /worker-browser-panel\.is-expanded \\{[^}]*position: fixed/s);
  assert.match(mvpJs, /mountWorkerBrowser/);
  assert.match(mvpJs, /targetUrl: elements\.portalUrl\.value\.trim\(\) \|\| null/);
  assert.match(mvpJs, /const providerMode = facadeSelected \? "facade_remote" : "local_cdp"/);
  assert.match(mvpJs, /state\.remoteBrowserSession = null/);
  assert.match(mvpJs, /authToken: state\.facadeAccessToken/);
...
test("user-friendly MVP app is a separate auth plus chat surface wired to real APIs", () => {
  ...
  assert.match(remoteBrowserJs, /\/api\/runtime\/browser\/screencast\/start/);
  assert.match(remoteBrowserJs, /\/openclaw\/claims-observe/);
  assert.match(remoteBrowserJs, /Continue read-only claim scan/);
  assert.match(remoteBrowserJs, /source_pointers/);
  assert.match(remoteBrowserJs, /normalizedPointFromEvent/);
  assert.match(remoteBrowserJs, /naturalWidth/);
  ...
  assert.match(mvpJs, /provider_network/);
  ...
  assert.match(mvpJs, /renderPharmacyFormularyBlock/);
  assert.match(mvpJs, /renderProviderNetworkBlock/);
  ...
  assert.match(mvpCss, /provider-network-grid/);
  assert.match(mvpCss, /provider-network-row/);
